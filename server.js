...
app.get('/api/warehouse', async (req, res) => {
  ...
  try {
    const { status, body } = await httpsGet(
      'tienda.mercadona.es',
      `/api/postal-codes/${cp}/?lang=es`,
      { 'Accept': 'application/json', 'User-Agent': 'Mozilla/5.0 (compatible)' }
    );
    if (status === 404 || (body && body.status === 'error')) {
      return res.status(404).json({ error: 'Tu código postal está fuera de la zona de reparto de Mercadona' });
    }
    const warehouse = body?.warehouse_id || body?.warehouse || DEFAULT_WAREHOUSE;
    res.json({ postal_code: cp, warehouse });
  } catch (err) {
    console.error(err);
    res.status(502).json({ error: 'No se pudo resolver el código postal', detail: err.message });
  }
});
